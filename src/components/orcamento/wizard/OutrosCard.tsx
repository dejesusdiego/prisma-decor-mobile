...
export function OutrosCard({
  ...
  const [categoria, setCategoria] = useState<string>(outro.descricao || 'Outros');
  ...
  useEffect(() => {
    const carregarMateriais = async () => {
      if (categoria !== 'Outros') {
        ...
        const categoriaBusca = categoriaMap[categoria];
        if (categoriaBusca) {
      ...
      // Resetar material selecionado ao mudar de categoria
      setMaterial(null);
  ...
  useEffect(() => {
    const carregarMaterial = async () => {
      if (outro.materialPrincipalId) {
        const { data } = await supabase
          .from('materiais')
          .select('*')
          .eq('id', outro.materialPrincipalId)
          .single();
        if (data) {
          setMaterial({
            id: data.id,
  ...
  const salvarOutro = async () => {
    ...
    try {
      ...
      const dadosOutro = {
        ...
        forro_id: null,
        trilho_id: null,
        material_principal_id: outro.materialPrincipalId || null,
      ...
      {expanded && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            <Label htmlFor={`categoria-${outro.id}`}>Categoria *</Label>
            <Select
              value={categoria}
              onValueChange={(value) => {
                setCategoria(value);
          ...
          {categoria !== 'Outros' && (
            <div className="space-y-2 md:col-span-2">
              <MaterialSelector
                ...
                materiais={materiais}
                value={outro.materialPrincipalId}
