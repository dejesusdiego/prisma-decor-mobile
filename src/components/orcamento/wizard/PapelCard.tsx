...
import { Copy, Trash2, ChevronDown, ChevronUp, RefreshCw, AlertTriangle, Info } from 'lucide-react';
...
export function PapelCard({
  ...
  const [coberturaPorRolo, setCoberturaPorRolo] = useState<number>(5); // m² por rolo default
  const [perdaPercent, setPerdaPercent] = useState<number>(10); // % de perda default
    ...
    const carregarMaterial = async () => {
      if (papel.materialPrincipalId) {
        ...
        if (data) {
          ...
          // Usar perda do material se disponível
  ...
  // Sincronizar dimensões do papel com o estado local
  useEffect(() => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setCoberturaPorRolo(parseFloat(e.target.value) || 5)}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    required
                    className={quantidadeMenorQueSugerido ? 'border-amber-500' : ''}
                  />
                  {rolosSugeridos > 0 && (
                    <Button
                      type="button"
                      ...
                      title="Aplicar quantidade sugerida"
                    >
